...
namespace PiggySync.Core
{
    internal class Syncronizer
    {
        ...
        private static readonly string rootPath = XmlSettingsRepository.Instance.Settings.SyncPath;
        ...
        {
            int i = 0;
            foreach (TCPPacket x in packets)
            {
                i++;
        ...
        public static void HandleSyncAsClientNoSSL(object hst) //TODO
        {
            try
            {
                ...
                FileChangesCollection remoteChanges = ChooseFilesToSync(rootFolder);
                foreach (FileDeletePacket x in remoteChanges.DeletedFiles)
                {
                    DeleteFile(x);
                }
                foreach (FileRequestPacket x in remoteChanges.FileRequests)
                {
                    SyncFile(x, stream);
                }
        ...
        private static void SyncFile(FileRequestPacket x, NetworkStream stream)
        {
            ...
            Int32 bytes;
            msg = new byte[2048];
            var fileStream = new FileStream(x.FilePath + @"\" + x.File.FileName, FileMode.Create, FileAccess.Write);
                //TODO optimize
            ...
            string path)
        {
            ...
            foreach (FileInfoPacket x in rootFolder.Files)
            {
            ...
            foreach (FileDeletePacket x in rootFolder.DeletedFiles)
            {
                throw new NotImplementedException();
                //TODO  add removed files
            }
            foreach (FolderInfoPacket x in rootFolder.Folders)
            {
                FolderInfoPacket localSubfolder = localFiles.Folders.Find(folder => folder.FolderName == x.FolderName);
                ...
                FileChangesCollection moreFiles = ChooseFilesToSync(x, localSubfolder, path + @"/" + x.FolderName);
        ...
        public static void SendFilePackets(SyncInfoPacket syncInfoPacket, NetworkStream stream)
        {
            ...
            List<FolderInfoPacket> folders = syncInfoPacket.Folders;
            foreach (FolderInfoPacket x in folders)
            {
        ...
        public static void HandleSyncAsServerNoSSL(object hst) //TODO
        {
            try
            {
                var host = (TcpClient) hst;
                NetworkStream stream = host.GetStream();
                SyncInfoPacket syncInfoPacket = FileManager.RootFolder;

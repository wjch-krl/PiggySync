...
namespace PiggySync.Core
{
	internal class Syncronizer
	{
		...
		public static void HandleSyncAsClientNoSsl (ITcpClient host, DateTime lastSyncDate, PiggySync.Core.SyncManager.Notifier notyfier)
		{
			try
			{
				using (INetworkStream stream = host.GetStream ())
				{
					...
					double step = 1 / (remoteChanges.DeletedFiles.Count + remoteChanges.FileRequests.Count);
		...
		public static void HandleSyncAsServerNoSsl (ITcpClient hst, PiggySync.Core.SyncManager.Notifier notyfier) //TODO
		{
			try
			{

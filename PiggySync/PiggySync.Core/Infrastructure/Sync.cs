    ...
    class Syncronizer
    {
        ...
        private static void SyncFile(FileRequestPacket x, NetworkStream stream)
        {
            ...
            while (size != 0)
            {
                ...
                if (bytes > size)
                {
                    fileStream.Write(msg, 0, (int)size);
                ...
                fileStream.Write(msg, 0, bytes); //TODO secure overflows etc...
                size -= (UInt32)bytes;
            }
            fileStream.Close();
            System.Diagnostics.Debug.WriteLine("Recived File: "+x.File.FileName);
        }
        ...
        private static void SendFile(FileRequestPacket fileReqPacket, NetworkStream stream)
        {
            ...
            try
            {
                ...
                byte[] lastPacket = (FilePacketCreator.CreatePacket(filePath, i, lastPacketSize));

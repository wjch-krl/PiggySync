...
namespace PiggySync.Core
{
    public class SyncManager
    {
        ...
        private readonly ConcurrentBag<PiggyRemoteHost> hosts = new ConcurrentBag<PiggyRemoteHost>();
        ...
        private void CleanHostsList()
        {
            ...
            {
                Thread.Sleep(100000);
                foreach (PiggyRemoteHost x in hosts)
                {
            ...
            do
            {
                ...
                switch (msg[0])
                {
                    ...
                    case 0:
                        Task.Factory.StartNew(() =>
                        {
                            uint tmp = BitConverter.ToUInt32(msg, 1); //udpdates ack with seq numm TODO describe better
        ...
        public bool DidReciveSyncNotyfy(byte[] msg)
        {
            ...
            foreach (PiggyRemoteHost x in hosts)
            {
                if (x.Name == name)
                {
                    byte[] packet = new PiggyACK(seqNumber).GetPacket();
            ...
            do
            {
                lock (serverLock) //TODO
                {
                    while (serverQueue.TryDequeue(out x))
                    {
                        ...
                        uint packetN = packetNum++;
                        var reqPacket = new SyncRequest(packetN);
                        byte[] packet = reqPacket.GetPacket();

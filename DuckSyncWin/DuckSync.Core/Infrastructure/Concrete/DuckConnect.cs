...
{
    public class PiggyConnect : IPiggyObserver
    {
        ...
        static PiggyRemoteHost me = new PiggyRemoteHost(IPUtils.LocalIPAddress(), "x");
                    ...
                    case 240:
                        Task.Factory.StartNew(() =>
                        {
                            try
                            {
                                ...
                                ((IPiggyObserver)observer).CreateNewConnection(remote); 
        ...
        public void CreateNewConnection(PiggyRemoteHost host)
        {
            ...
            if (!hosts.Contains(host) && PiggyConnect.Me.Name != host.Name)
            {
                foreach (var x in hosts)
                {
